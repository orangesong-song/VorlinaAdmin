# -*- coding: utf-8 -*-
"""
vadmin-020c · 收口：从「＋ 上传新图 / 从图库追加」入口上传的图应**直接进本型号画廊**，
并把验收 IMG3 改成断这条真实路径（此前 IMG3 断言前提错了：上传不自动进画廊，
缩略图条按设计不显示它 —— 红得对，是设计缺口，不是断言写错）。

设计判据（两者意图不同，行为必须不同）：
  · 从「＋ 上传新图 / 从图库追加」上传 → 意图是「给这个型号加图」⇒ 自动追加进画廊（画廊空则设主图）
  · 从画廊某格的「选图」上传       → 意图是「替换这一格」  ⇒ 不自动追加，只登记进图库
"""
import io

n = 0
def rep_file(F, old, new, label):
    global n
...
IX = 'index.html'
VP = 'tools/verify-picker.js'

# ── ① 新函数 ensureGalleryAdd ───────────────────────────────────────
rep_file(IX,
"""function edThumbUrl(fn, map){""",
"""function ensureGalleryAdd(name){
  /* vadmin-020c：从「＋ 上传新图」入口上传 = 意图「给这个型号加图」⇒ 直接进画廊 */
  if (!ED || ED.path !== 'data/products.json' || !name) return;
  const p0 = edGet(ED.doc, ED.sub) || {};
  if (!p0.images) p0.images = { hero: '', usable: [] };
  if (!Array.isArray(p0.images.usable)) p0.images.usable = [];
  if (p0.images.usable.indexOf(name) < 0) p0.images.usable.push(name);
  if (!p0.images.hero) p0.images.hero = name;
}
function edThumbUrl(fn, map){""",
'① 新增 ensureGalleryAdd')
...
"""        /* vadmin-020：就地登记进本型号 imagery 图库 —— 否则主图下拉列不出、缩略图「图库未匹配」、官网取不到 url */
        ensureInImagery(upName);
        if (PICK && PICK.path === null) ensureGalleryAdd(upName);   /* 从「＋上传新图」入口来的：直接进画廊 */""",
'② 上传成功：按入口区分是否自动进画廊')
...
"""  await mediaLoad();
  MEDIA.busy = false;
  /* vadmin-020：编辑页的缩略图条 / 主图下拉立刻反映新图（否则关掉弹层还以为没传上） */
  if (ED && typeof edRender === 'function'){ try { edRender() } catch(e){} }""",
'③ 上传完成后刷新编辑页')

# ── ④ 验收：IMG 段改为走真实路径（＋上传新图 入口 → 自动进画廊）──────
old_img1 = """  check('IMG1 上传的文件名登记进 imagery 图库[' + img1.sku + '] 且带 /assets/img/ url',
    !!img1.hit && img1.hit.url === '/assets/img/' + UP_NAME,
    JSON.stringify(img1.hit || null));"""
assert VP.count(old_img1) == 1, 'IMG1 锚点命中 %d 次' % VP.count(old_img1)
...
new_img = """  /* ── IMG · 走真实路径：「＋ 上传新图 / 从图库追加」入口上传（vadmin-020）──
  ...
  await page.evaluate(async name => { await mediaLoad(); renderPickGrid(); }, UP2);
...
n += 1
print('  ✓ ④ IMG 段整体替换为真实路径版（IMG0–IMG6）')

print('\nOK · vadmin-020c 完成 %d 处改动' % n)
